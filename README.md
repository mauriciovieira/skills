...
- **[review-with-jev](./skills/engineering/review-with-jev/SKILL.md)** - Staged, evidence-first review through Jev (TypeSafe's System-1 model) over the upstream `jev_review` MCP tool - scrub the evidence, route which dimensions matter for this change, rank the scores below deterministic checks, and stop early instead of chasing a number.
- **[setup-mauricio-vieira-skills](./skills/engineering/setup-mauricio-vieira-skills/SKILL.md)** — Scaffold the per-repo config (issue tracker, triage label vocabulary, domain doc layout) that the other engineering skills consume. Run once per repo before using `diagnosing-bugs`, `tdd`, `triage`, or `to-tickets`.
