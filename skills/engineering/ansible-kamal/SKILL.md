...
11. **Secret command** - a command on PATH, or an absolute path, optionally with fixed arguments, that prints the secret named by its last argument to stdout and nothing else. It is invoked from the project root AND from `infra/ansible`, so a relative path would mean two different files and is rejected; a leading `~` and a `$` are rejected for the same reason, since make and `/bin/sh` expand them differently. It is split on whitespace to separate the command from its fixed arguments, so a command path containing a space cannot work. Wrap anything that needs environment or pipelines in a small script, put it on PATH, and name that.
...
| `__SECRET_CMD__` | secret-printing command on PATH, e.g. `secret` |
...
For `ENV_MODE=single` leave `DOMAIN_STAGING` empty (or unset). `SECRET_CMD` is rejected if it is a relative path, starts with `~`, or contains `$` - each of those resolves differently depending on which directory invokes it.
...
6. Run `DEPLOY_SSH_KEY="$(secret __SECRET_NAMESPACE__/deploy_ssh_public_key)" make bootstrap` (root reachable).
